...
<div class="wrap">
    ...
    <?php wp_nonce_field( 'bbse_franchise_config_save', '_bbse_franchise_nonce' ); ?>
      <input type="hidden" name="action" id="action" value="bbse_franchise_config_save">
        ...
        <tr class="form-field daummap <?php echo (!empty($configs['useapi']) && $configs['useapi'] == 'daum')?'ondisplay':'offdisplay'?>">
            <th scope="row"><label for="daumapikey"><?php echo __('Daum Map API', 'bbsefranchise')?></label></th>
            <td>
                <label class="note"><input name="daumapikey" type="text" id="daumapikey" value="<?php echo !empty($configs['daumapikey']) ? stripslashes(esc_attr($configs['daumapikey'])) : ''?>" maxlength="60" placeholder="API Key" style="width:30%;" title="API Key입니다."></label>
