...
class FRANCHISE
{
     ...
     * db tables create
     */
    protected function _dbdelta_bbse_franchise()
    {
        global $wpdb;
        ...
        //create config table
        $query = '';
        $query = " CREATE TABLE `{$this->table_config}` (
                        `uid` int(11) NOT NULL AUTO_INCREMENT COMMENT '고유번호',
                        `prefix` varchar(255) NOT NULL COMMENT '사이트구분',
                        `pageid` int(11) NOT NULL COMMENT '삽입페이지 번호',
                        `listtitle` varchar(255) NOT NULL COMMENT '목록 제목',
                        `listcomment` varchar(255) NOT NULL COMMENT '목록 코멘트',
                        `usemapnavi` enum('y','n') NOT NULL DEFAULT 'y' COMMENT '지도네비 사용여부',
                        `template` varchar(255) NOT NULL COMMENT '사용스킨',
                        `category` text NOT NULL COMMENT '카테고리',
                        `perpage` tinyint(4) NOT NULL COMMENT '페이지당 목록수',
                        `useapi` varchar(10) NOT NULL COMMENT 'api종류',
                        `naverapiid` varchar(255) NOT NULL COMMENT '네이버아이디',
                        `naverapisecret` varchar(255) NOT NULL COMMENT '네이버시큐리티',
                        `daumapikey` varchar(255) NOT NULL COMMENT '다음지도',
                        `var1` text NOT NULL COMMENT '예비1',
                        `var2` text NOT NULL COMMENT '예비2',
                        `var3` text NOT NULL COMMENT '예비3',
                        `var4` text NOT NULL COMMENT '예비4',
                        `var5` text NOT NULL COMMENT '예비5',
                        `var6` text NOT NULL COMMENT '예비6',
                        `var7` text NOT NULL COMMENT '예비7',
                        `var8` text NOT NULL COMMENT '예비8',
                        `var9` text NOT NULL COMMENT '예비9',
                        `var10` text NOT NULL COMMENT '예비10',
                        PRIMARY KEY (`uid`),
        ...
        //create list table
        $query = '';
        $query = " CREATE TABLE `{$this->table_list}` (
                        `uid` int(11) NOT NULL AUTO_INCREMENT COMMENT '키',
                        `prefix` varchar(255) NOT NULL COMMENT '사이트구분',
                        `hide` enum('Y','N') NOT NULL DEFAULT 'N' COMMENT '보임',
                        `category` varchar(255) NOT NULL COMMENT '카테고리번호',
                        `branchname` varchar(255) NOT NULL COMMENT '지점이름',
                        `addresstype` enum('J','R') NOT NULL DEFAULT 'J' COMMENT '주소종류(지번,도로)',
                        `languagetype` enum('K','E') NOT NULL DEFAULT 'K' COMMENT '표시 주소언어',
                        `zipcode` varchar(8) NOT NULL COMMENT '우편번호',
                        `address1` varchar(255) NOT NULL COMMENT '주소1',
                        `address2` varchar(255) NOT NULL COMMENT '주소2',
                        `mapaddress` varchar(255) NOT NULL COMMENT '지도용 주소',
                        `conutry` varchar(30) NOT NULL COMMENT '나라',
                        `sido` varchar(30) NOT NULL COMMENT '도시',
                        `sigungu` varchar(255) NOT NULL COMMENT '시군구',
                        `bname` varchar(30) NOT NULL COMMENT '지역',
                        `phone` varchar(30) NOT NULL COMMENT '전화번호',
                        `mobile` varchar(30) NOT NULL COMMENT '모바일번호',
                        `fax` varchar(30) NOT NULL COMMENT '팩스번호',
                        `email` varchar(255) NOT NULL COMMENT '이메일주소',
                        `tag` varchar(255) NOT NULL,
                        `memo` text NOT NULL COMMENT '내용',
                        `regdate` varchar(10) NOT NULL COMMENT '등록일',
                        `var1` text NOT NULL COMMENT '예비1',
                        `var2` text NOT NULL COMMENT '예비2',
                        `var3` text NOT NULL COMMENT '예비3',
                        `var4` text NOT NULL COMMENT '예비4',
    ...
    */
    ...
    public function config_save()
    {
        ...
        if ($pq_useapi != 'naver' && $pq_useapi != 'daum') {
            ...
            if ($useapi == 'daum' && empty($pq_daumapikey)) {
